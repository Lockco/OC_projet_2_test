...
for link in links_nav:
    ...
    books_data = {'title': title(),'rate':rate(),'price':catch_book_price(), 'stock': catch_stock_availability(), 'url':catch_book_url()}

    for i in range(nav):
        
        links = nav
        print()
        nav_list.append(links)
        df = pd.DataFrame(books_data).to_csv('category'+str(i+1)+'.csv', encoding = 'utf-8')
